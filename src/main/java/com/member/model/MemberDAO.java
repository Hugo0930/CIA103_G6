...
//	private static final String GET_ONE_STMT = "SELECT member_id, mem_lv_id, mem_name,mem_uid, mem_bth, mem_gender, mem_email, mem_tel, mem_add, mem_acc, mem_pw, mem_status  FROM member where mem_id= ?";
	...
	public static MemberVO selectAdmin(String name, String pwd) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				mem = new MemberVO(rs.getInt("MEM_ID"), rs.getByte("MEM_LV_ID"), rs.getString("MEM_NAME"),
						rs.getString("MEM_UID"), rs.getDate("MEM_BTH"), rs.getByte("MEM_Gender"),
						rs.getString("MEM_EMAIL"), rs.getString("MEM_TEL"), rs.getString("MEM_ADD"),
						rs.getString("MEM_ACC"), rs.getString("MEM_PW"), rs.getByte("MEM_STATUS"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
