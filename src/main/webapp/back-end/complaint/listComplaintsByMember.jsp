        ...
        <table class="table table-bordered table-striped table-hover">
            ...
            <tbody>
                <c:choose>
                    <c:when test="${not empty complaintList}">
                        <c:forEach var="complaintVO" items="${complaintList}">
                            <tr>
                                ...
                                <td>
                                    <!-- 修改狀態按鈕 -->
                                    <form action="<%= request.getContextPath() %>/complaintServlet?action=getOne_For_Update&complaintId=${complaintVO.complaintId}" method="post" style="display:inline;">
                                        <input type="hidden" name="complaintId" value="${complaintVO.complaintId}">
                                        <button type="submit" class="btn btn-sm btn-primary">修改狀態</button>
                                    </form>
                                </td>
                            </tr>
                        </c:forEach>
                    ...
                    <c:otherwise>
                        <tr>
                            <td colspan="8" class="text-center">無任何申訴案件記錄</td>
                        </tr>
                    </c:otherwise>
                </c:choose>
            </tbody>
