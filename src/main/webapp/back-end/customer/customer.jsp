...
<link rel="stylesheet" href="${pageContext.request.contextPath}/back-end/customer/css/customer.css">    
</head>
<body>
    <!-- 漢堡 -->
    ...
    <!-- 現在位置 -->
    <div class="container">
        <h2><i class="fa-solid fa-house"></i>客服Q&A</h2>
        <hr>
        <div class="lookup">
	        ...
	        <select id="sort_type">
	        	...
	        	<option>案件編號-遞增排序</option>
	        	<option>案件編號-遞減排序</option>
	        </select>
        </div>
        <table class="main">
        	<thead>
        	...
        	<tbody>
        		<c:forEach var="userCase" items="${requestScope.resultList}">
	        		<tr>
	        			...
	        			<td>
						    <fmt:parseDate value="${userCase.createdAt}" var="fmtDate1" pattern="yyyy-MM-dd HH:mm:ss" />
							<fmt:formatDate value="${fmtDate1}" pattern="yyyy/MM/dd HH:mm:ss" />
	        			</td>
	        			<td>
							<fmt:parseDate value="${userCase.updatedAt}" var="fmtDate2" pattern="yyyy-MM-dd HH:mm:ss" />
	    ...
	    <div class="reply_view" >
	    	<form action="${pageContext.request.contextPath}/CustomerServiceServlet" method="post">
		    	...
		    	<input type="hidden" name="action" value="reply_cs_case">
	    	</form>
	    </div>
	    <%-- 查看頁面 --%>
	    <div class="see_view_block">
		    <div class="see_view">
		    	<table>
	 		    	<tr>
		    			<td>案件編號 : </td>
    ...
    <script src="${pageContext.request.contextPath}/back-end/customer/js/customer.js"></script>
</body>
</html>